...
class VITS(L.LightningModule):
    ...
    def training_step(self, batch):
        y, spec, spec_lengths, f0, text, text_lengths, sid, lang_id = batch
        ...
        ) = self.net_g(text, text_lengths, spec, spec_lengths, f0, sid)
        ...
        loss_kl = kl_loss(z_q_dur, logs_q_dur, m_p_dur, logs_p_dur, z_mask)
        dur_logits_fake = self.net_dd(h_text, x_mask, logw_hat, g)
        loss_dur_adv = duration_generator_adversarial_loss(dur_logits_fake)

        loss_g = loss_stft * 45.0 + loss_adv + loss_feat + loss_kl + loss_f0 + loss_dp + loss_sdp + loss_dur_adv
        ...
        self.log("generator/kl divergence", loss_kl.item())
